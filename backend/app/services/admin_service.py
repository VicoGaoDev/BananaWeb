...
def _redeem_amount_yuan(credit_amount: int, sale_amount_fen: int | None = None) -> float:
    return _redeem_unit_price_yuan(credit_amount, sale_amount_fen)
...
def get_analytics_revenue_timeseries(
    ...
    *,
    ...
    redeem_rows = (
        db.query(CreditRedeemKey.used_at, CreditRedeemKey.credit_amount, CreditRedeemKey.sale_amount_fen)
        .filter(
            CreditRedeemKey.used_at.isnot(None),
            CreditRedeemKey.is_gift.is_(False),
            CreditRedeemKey.used_at >= current_start,
    ...
    for used_at, credit_amount, sale_amount_fen in redeem_rows:
        if used_at is None:
            continue
        bucket = _bucket_start(_to_local_datetime(used_at), granularity)
        if bucket not in bucket_map:
            continue
        bucket_map[bucket]["redeem_amount"] += _redeem_amount_yuan(int(credit_amount or 0), sale_amount_fen)
